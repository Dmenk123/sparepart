...
class Master_agen extends CI_Controller {
	...
	{
		$list = $this->m_agen->get_datatable_user();
		$data = array();
		$no =$_POST['start'];
	...
	private function rule_validasi($is_update=false, $skip_pass=false)
	{
		...
		if ($this->input->post('nama_pers') == '') {
			$data['inputerror'][] = 'nama_pers';
            $data['error_string'][] = 'Wajib Mengisi Nama Perusahaan';
            $data['status'] = FALSE;
		}

		if ($this->input->post('produk') == '') {
			$data['inputerror'][] = 'produk';
            $data['error_string'][] = 'Wajib Mengisi Produk';
            $data['status'] = FALSE;
		}
